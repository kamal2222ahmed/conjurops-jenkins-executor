default['jenkins']['master']['install_method'] = 'package'
default['jenkins']['master']['home'] = '/mnt/jenkins'

default['docker']['package']['repo_url'] = 'https://get.docker.io/ubuntu'

default['conjur']['version'] = '4.17.0'
default['conjur']['configuration']['account'] = 'conjurops'
default['conjur']['configuration']['appliance_url'] = 'https://conjur-master.itp.conjur.net/api'
default['conjur']['configuration']['plugins'] = []
default['conjur']['configuration']['ssl_certificate'] = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"

default['vagrant']['url'] = 'https://dl.bintray.com/mitchellh/vagrant/vagrant_1.7.0_x86_64.deb'
default['vagrant']['checksum'] = '890a5b0ec8a9b91f96580dd36d8eea3a95707b56460cab902753a2f2af93b39d'

normal['postgresql']['password']['postgres'] = if File.exists?('/etc/postgresql.password')
  File.read('/etc/postgresql.password')
else
  require 'securerandom'
  SecureRandom.hex(16)
end
